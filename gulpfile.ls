require! <[gulp gulp-util express connect-livereload gulp-jade tiny-lr gulp-livereload path gulp-ruby-sass gulp-angular-templatecache]>
require! <[gulp-if gulp-livescript gulp-less gulp-concat gulp-json-editor gulp-commonjs gulp-insert streamqueue gulp-uglify gulp-bower gulp-filter gulp-bower-files]>

gutil = gulp-util

app = express!
lr = tiny-lr!

build_path = '_public'
production = true if gutil.env.env is \production

gulp.task 'html' ->
  gulp.src 'app/**/*.jade'
    .pipe gulp-jade!
    .pipe gulp.dest "#{build_path}"
    .pipe gulp-livereload lr

gulp.task 'html:template', <[html]>, ->
  gulp.src 'app/partials/**/*.jade'
    .pipe gulp-jade!
    .pipe gulp-angular-templatecache 'app.templates.js' do
      base: process.cwd()
      filename: 'app.templates.js'
      module: 'app.templates'
      standalone: true
    .pipe gulp.dest "#{build_path}/js"
    .pipe gulp-livereload lr

gulp.task 'bower' ->
  gulp-bower!

gulp.task 'js:vendor', <[bower]>, ->
  bower = gulp-bower-files!
    .pipe gulp-filter -> it.path is /\.js$/

  vendor = gulp.src 'vendor/scripts/*.js'

  streamqueue { +objectMode }
    .done bower, vendor
    .pipe gulp-concat 'vendor.js'
    .pipe gulp-if production, gulp-uglify()
    .pipe gulp.dest "#{build_path}/js"

gulp.task 'js:app', ->
  env = gulp.src 'app/**/*.jsenv'
    .pipe gulp-json-editor (json) ->
      for key of json when process.env[key]?
        json[key] = that
      json
    .pipe gulp-insert.prepend 'module.exports = '
    .pipe gulp-commonjs!

  app = gulp.src 'app/**/*.ls'
    .pipe gulp-livescript({+bare}).on 'error', gutil.log

  js = gulp.src 'app/**/*.js'

  streamqueue { +objectMode }
    .done env, app, js
    .pipe gulp-concat 'app.js'
    .pipe gulp-if production, gulp-uglify()
    .pipe gulp.dest "#{build_path}/js"
    .pipe gulp-livereload lr

gulp.task 'css', ->
  #compress = production
  gulp.src ['app/styles/*.sass', 'app/styles/*.scss']
    .pipe gulp-ruby-sass compass: true
    .pipe gulp-concat 'app.css'
    .pipe gulp.dest "#{build_path}/css"
    .pipe gulp-livereload lr

gulp.task 'assets', ->
  gulp.src 'app/assets/**/*'
    .pipe gulp.dest "#{build_path}"

gulp.task 'server', ->
  app.use connect-livereload!
  app.use express.static path.resolve "#build_path"
  app.listen 3000
  gulp-util.log 'Listening on port 3000'

gulp.task 'watch', ->
  lr.listen 35729, ->
    return gutil.log it if it
  gulp.watch 'app/**/*.jade', <[html:template]>
  gulp.watch 'app/**/*.sass', <[css]>
  gulp.watch 'app/**/*.ls', <[js:app]>

gulp.task 'build' <[html:template bower assets js:vendor js:app css]>
gulp.task 'dev', <[build server watch]>
gulp.task 'default', <[build]>
 